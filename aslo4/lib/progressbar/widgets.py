...
def create_wrapper(wrapper):
    ...
    if isinstance(wrapper, six.string_types):
        ...
        raise RuntimeError(
            "Pass either a begin/end string as a tuple or a" " template string with {}"
        )
...
class FileTransferSpeed(FormatWidgetMixin, TimeSensitiveWidgetBase):
    ...
    def __call__(self, progress, data, value=None, total_seconds_elapsed=None):
        ...
        if power == 0 and scaled < 0.1:
            ...
            return FormatWidgetMixin.__call__(self, progress, data, self.inverse_format)
        else:
            data["scaled"] = scaled
            data["prefix"] = self.prefixes[power]
            return FormatWidgetMixin.__call__(self, progress, data)
...
class AnimatedMarker(TimeSensitiveWidgetBase):
    ...
    def __call__(self, progress, data, width=None):
        ...
        if self.fill:
            ...
            fill = self.fill(progress, data, width - progress.custom_len(marker))
        else:
            fill = ""
...
class SimpleProgress(FormatWidgetMixin, WidgetBase):
    ...
    def __call__(self, progress, data, format=None):
        ...
        formatted = FormatWidgetMixin.__call__(self, progress, data, format=format)
...
class ReverseBar(Bar):
    ...
    def __init__(
        self, marker="#", left="|", right="|", fill=" ", fill_left=False, **kwargs
    ):
        """Creates a customizable progress bar.
...
class BouncingBar(Bar, TimeSensitiveWidgetBase):
    ...
    def __call__(self, progress, data, width):
        ...
        if width:  # pragma: no branch
            value = int(data["total_seconds_elapsed"] / self.INTERVAL.total_seconds())
...
class FormatCustomText(FormatWidgetMixin, WidgetBase):
    ...
    def __call__(self, progress, data):
        return FormatWidgetMixin.__call__(self, progress, self.mapping, self.format)
    ...
    def __init__(
        ...
        **kwargs
        ...
        MultiRangeBar.__init__(
            self, name=name, markers=list(reversed(markers)), **kwargs
        )
    ...
    def get_values(self, progress, data):
        ...
        for progress in data["variables"][self.name] or []:
            ...
            if progress < 0 or progress > 1:
                raise ValueError(
                    "Range value needs to be in the range [0..1], got %s" % progress
                )
...
class Variable(FormatWidgetMixin, VariableMixin, WidgetBase):
    ...
    def __init__(
        self, name, format="{name}: {formatted_value}", width=6, precision=3, **kwargs
    ):
...
class CurrentTime(FormatWidgetMixin, TimeSensitiveWidgetBase):
    ...
    def __init__(
        self, format="Current Time: %(current_time)s", microseconds=False, **kwargs
    ):
